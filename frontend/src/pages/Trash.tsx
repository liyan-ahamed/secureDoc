    ...
    <div className="flex min-h-[calc(100vh-3.5rem)]">
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 py-5 sm:py-6">
        ...
        <section className="glass-panel rounded-2xl overflow-hidden">
          <div className="hidden sm:grid sm:grid-cols-[1fr_110px_180px_200px] gap-3 px-4 py-3 bg-bg border-b border-border text-xs font-medium text-muted uppercase tracking-wider"><span>Name</span><span>Size</span><span>Deleted</span><span className="text-right">Actions</span></div>
...
const TrashRow = ({ item, busy, onRestore, onDelete }: { item: TrashItem; busy: boolean; onRestore: () => void; onDelete: () => void }) => {
  ...
  return <div className="grid grid-cols-1 sm:grid-cols-[1fr_110px_180px_200px] gap-3 items-center px-4 py-4 hover:bg-bg transition-colors"><div className="min-w-0 flex items-center gap-3"><Icon className="w-5 h-5 shrink-0 text-accent" /><p className="text-sm font-medium text-primary truncate">{item.name}</p></div><span className="text-sm text-muted"><span className="sm:hidden font-medium text-primary mr-2">Size:</span>{item.type === 'file' ? formatSize(item.size) : '—'}</span><span className="text-sm text-muted"><span className="sm:hidden font-medium text-primary mr-2">Deleted:</span>{formatDate(item.deletedAt)}</span><div className="justify-self-start sm:justify-self-end flex flex-wrap gap-2"><button disabled={busy} onClick={onRestore} className="min-h-11 py-1.5 px-2.5 rounded-md border border-border text-xs font-medium text-primary hover:bg-bg disabled:opacity-50 cursor-pointer flex items-center gap-1"><RotateCcw className="w-3.5 h-3.5" />Restore</button><button disabled={busy} onClick={onDelete} className="min-h-11 py-1.5 px-2.5 rounded-md border border-danger/40 text-xs font-medium text-danger hover:bg-danger/10 disabled:opacity-50 cursor-pointer flex items-center gap-1"><Trash2 className="w-3.5 h-3.5" />Delete Forever</button></div></div>;
};
